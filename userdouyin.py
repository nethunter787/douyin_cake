# -*- coding: utf-8 -*-
"""
Created on Fri Apr 14 00:05:52 2023
...
user_url = 'https://www.douyin.com/user/MS4wLjABAAAAV2Kw3FNI3rkcIuyeP_qUh7ndAurPOi9Lt1iALq8kT-Oq2npccNicImaIDE8nXENc'
...
def getUserCakeUrl(user_url,max_cursor = 0,count=20):
    sec_user_id = urllib.parse.urlparse(user_url).path.split('/')[-1]
    p = {'sec_user_id':sec_user_id,
         'max_cursor':max_cursor,
         'aid':6383,
         'count':count,
         'mstoken':'Nj_ZPDsOI_W6JWK02i7NDBuS9asQnAUeFdOCbUlI7D0KRj_v6O8tllNKJeO88gG6u-xoztTJ_cBTnqhTB6A6o9agfiA3pFyJIyr58vV9GnOKk8kNefQXAA==',
         'webid':7224530453531837967,
         }
    api = 'https://www.douyin.com/aweme/v1/web/aweme/post/?%s'%(urllib.parse.urlencode(p)) # 该接口可以获取视频链接
    ...
    cake_json = getjson(api_xbogus_signed)
    return cake_json
...
while(usercakenum < 1000):
    time.sleep(0.2)
    res = getUserCakeUrl(user_url,max_cursor)
    if(len(res['aweme_list']) == 0):
        break
    else:
        print("[-] 下载%d个视频中。"%len(res['aweme_list']))
    max_cursor = res['max_cursor'] # 下一个指针
    for i in range(len(res['aweme_list'])):
        ...
        video_id            = res['aweme_list'][i]['statistics']['aweme_id']
        ...
        video_folder_path   = os.path.join(work_path,video_folder_name)
        ...
        if video_exist_file: # 如果文件下已经有该视频文件了，跳过下载
            ...
            continue
        urlretrieve_evo_headers = [('referer', user_url),('User-agent', userAgent)] # 添加上下载头，防止下载失败
        # -------------------------- 下载视频封面 --------------------------
